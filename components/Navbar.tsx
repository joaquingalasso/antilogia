...
interface NavbarProps {
    theme: Theme;
    toggleTheme: () => void;
    currentView: 'gallery' | 'story' | 'about';
}
...
export const Navbar = ({ theme, toggleTheme, currentView }: NavbarProps) => {
    const { t } = useLanguage();
    const isGallery = currentView === 'gallery' || currentView === 'story';

    // This handler prevents the default link navigation and manually updates the hash.
    // This is a workaround for the AI Studio preview environment which can incorrectly
    // block hash-based client-side routing.
    ...
    return (
        <nav className="site-navbar">
            ...
            <div className="navbar-links">
                <a href="#/about" className={`navbar-link ${currentView === 'about' ? 'active' : ''}`} onClick={(e) => handleNavClick(e, '#/about')}>
                    <span className="navbar-icon-wrapper"><WhoIcon /></span>
                    <span className="navbar-link-text">{t('navWho')}</span>
                </a>
                <a href="https://joaquingalasso.com" target="_blank" rel="noopener noreferrer" className="navbar-link">
                    <span className="navbar-icon-wrapper"><WhatIcon /></span>
                    <span className="navbar-link-text">{t('navWhat')}</span>
                </a>
                <a href="#/" className={`navbar-link ${isGallery ? 'active' : ''}`} aria-current={isGallery ? 'page' : undefined} onClick={(e) => handleNavClick(e, '#/')}>
